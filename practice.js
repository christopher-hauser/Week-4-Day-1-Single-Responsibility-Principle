...
const bakePie = (recipe, pieQuantity, pieType) => {
...
const costOfPie = recipe => {
  let pie = recipe.reduce(function(prev, current) {
    return prev + current.cost;
  }, recipe[0].cost);
  return pie;
}

const revenueAndProfitMargin = (recipe, pieQuantity, profitMargin) => {
  // Calculate the total cost of all the pies
  let totalCost = costOfPie(recipe) * pieQuantity;
...
function bakeAndSellPies(pieType, pieQuantity, profitMargin) {
  ...
  bakePie(recipe, pieQuantity, pieType);

  console.log(`Cost per pie: ${costOfPie(recipe)}`);

  revenueAndProfitMargin(recipe, pieQuantity, profitMargin);
